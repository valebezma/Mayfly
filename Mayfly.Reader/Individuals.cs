...
namespace Mayfly.Reader
{
    public partial class Individuals : Form
    {
        ...
        protected bool FixTotals { get; set; }
        protected bool AutoIncreaseTotal { get; set; }
        protected bool AutoDecreaseTotal { get; set; }
        ...
        public int DetailedQuantity
        {
            get
            {
                ...
                foreach (DataGridViewRow gridRow in spreadSheetLog.Rows)
                {
                    ...
                    if (gridRow.Cells[ColumnIdenticCount.Index].Value == null)
                    {
                        result++;
                    }
                    else
                    {
                        result += (int)gridRow.Cells[ColumnIdenticCount.Index].Value;
                    }
                }
        ...
        public Individuals()
        {
            ...
            ColumnIdenticCount.ValueType = typeof(int);
            ColumnComments.ValueType = typeof(string);

            numericUpDownMass.Enabled = numericUpDownQuantity.Enabled = !FixTotals;
            spreadSheetLog.CurrentCell = spreadSheetLog.FirstClearCell();
            UpdateTotals();


            ToolStripMenuItemSaveVars.Click += ToolStripMenuItemSaveVars_Click;
            buttonOK.Click += buttonOK_Click;

            AddVariableMenuItems(UserSettings.AddtVariables);
            AddVariableColumns(UserSettings.CurrentVariables);

            FixTotals = UserSettings.FixTotals;
            AutoIncreaseTotal = UserSettings.AutoIncreaseBio;
            AutoDecreaseTotal = UserSettings.AutoDecreaseBio;            
        ...
        public Individuals(Data.LogRow logRow) : this()
        {
            ...
            Text = string.Format(Wild.Resources.Interface.IndLog,
            ...
            else
            {
                ...
                ColumnIdenticCount.Visible = !spreadSheetLog.IsEmpty(ColumnIdenticCount);
        ...
        public void SetColumns(DataGridViewColumn columnSpecies, DataGridViewColumn columnQuantity, DataGridViewColumn columnMass)
        {
        ...
        private void UpdateTotals()
        {
            ...
            if (AutoDecreaseTotal && MassDifference < 0)
            {
                Mass += MassDifference;
            }

            if (AutoDecreaseTotal && QuantityDifference < 0)
            {
        ...
        protected void AddVariableMenuItems(string[] variables)
        {
            int newVarPostion = 6;
            ...
            foreach (string addtVar in variables)
            {
        ...
        protected void AddVariableColumns(string[] variables)
        {
            foreach (string currVar in variables)
            {
        ...
        Saving:
            ...
            if (gridRow.Cells[ColumnIdenticCount.Index].Value == null)
            {
                individualRow.SetIdenticCountNull();
            }
            else
            {
                int identicCount = (int)gridRow.Cells[ColumnIdenticCount.Index].Value;
                if (identicCount > 1) individualRow.IdenticCount = identicCount;
            }
        ...
        private DataGridViewRow InsertIndividualRow(Data.IndividualRow individualRow)
        {
            ...
            {
                gridRow.Cells[ColumnIdenticCount.Index].Value = individualRow.IdenticCount;
            }
        ...
        {
            spreadSheetLog.EnsureVisible(ColumnIdenticCount);
        }
        ...
        {
            toolTipAttention.ToolTipTitle = Wild.Resources.Messages.MassInequal;
        ...
        private void pictureBoxWarningQ_MouseHover(object sender, EventArgs e)
        {
            if (DetailedQuantity > Quantity)
            {
                toolTipAttention.ToolTipTitle = Wild.Resources.Messages.QuantityInequal;
                toolTipAttention.Show(string.Format(Wild.Resources.Messages.QuantitySetEqual,
                    DetailedQuantity, Quantity), numericUpDownQuantity, 0, numericUpDownQuantity.Height);
            }
            else
            {
                toolTipAttention.ToolTipTitle = Wild.Resources.Messages.QuantityEqual;
                toolTipAttention.Show(string.Format(Wild.Resources.Messages.QuantityEqualDetails,
                ...
                foreach (DataGridViewRow gridRow in spreadSheetLog.SelectedRows)
                {
                    ...
                    for (int i = 0; i < copies; i++)
                    {
                        ...
                        foreach (DataGridViewColumn gridColumn in new DataGridViewColumn[] {
                            ColumnLength, ColumnMass, ColumnSex })
                        {
        ...
        private void buttonReport_Click(object sender, EventArgs e)
        {
            ...
            report.End("© {0} {1}", LogRow.CardRow.When.Year, LogRow.CardRow.Investigator);
            report.Run();
        }
