...
namespace Mayfly.TaskDialogs
{
    ...
    public class TaskDialogButton : TaskDialogItem
    {
        ...
        /// </summary>
        /// <value>
        /// One of the <see cref="Mayfly.TaskDialogs.ButtonType"/> values that indicates the type of the button. The default value
        /// is <see cref="Mayfly.TaskDialogs.ButtonType.Custom"/>.
        /// </value>
        ...
        ///   is <see cref="Mayfly.TaskDialogs.ButtonType.Custom"/>. For other button types, it is ignored.
        /// </para>
        /// <para>
            ...
            set
            {
                if( base.Id != value )
                {
                    if( _type != ButtonType.Custom )
                        throw new InvalidOperationException(Resources.TaskDialogs.NonCustomTaskDialogButtonIdError);
                    base.Id = value;
                }
            }
        }
            ...
            if( type != ButtonType.Custom && Owner != null )
            {
                foreach( TaskDialogButton button in Owner.Buttons )
                {
                    if( button != this && button != itemToExclude && button.ButtonType == type )
                        throw new InvalidOperationException(Resources.TaskDialogs.DuplicateButtonTypeError);
                }
            }
        }
    }
