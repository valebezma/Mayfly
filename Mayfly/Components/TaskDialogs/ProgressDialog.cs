    ...
    public partial class ProgressDialog : Component
    {
        ...
        public ProgressDialog(IContainer container)
        {
            ...
            if( !NativeMethods.IsWindowsVistaOrLater )
                Animation = AnimationResource.GetShellAnimation(Mayfly.TaskDialogs.ShellAnimation.FlyingPapers);
        }
        ...
        /// </summary>
        /// <value>
        /// One of the values of <see cref="Mayfly.TaskDialogs.ProgressBarStyle"/>. 
        /// The default value is <see cref="Mayfly.TaskDialogs.ProgressBarStyle.ProgressBar"/>.
        /// </value>
        /// <remarks>
        /// <note>
        ...
        ///   progress bar will be hidden completely if this property is <see cref="Mayfly.TaskDialogs.ProgressBarStyle.MarqueeProgressBar"/>.
        /// </note>
        /// <para>
        ///   When this property is set to <see cref="Mayfly.TaskDialogs.ProgressBarStyle.ProgressBar" />, use the <see cref="ReportProgress(int)"/> method to set
        ///   the value of the progress bar. When this property is set to <see cref="Mayfly.TaskDialogs.ProgressBarStyle.MarqueeProgressBar"/>
        ...
        /// </para>
        /// <para>
        ///   This method has no effect is <see cref="ProgressBarStyle"/> is <see cref="Mayfly.TaskDialogs.ProgressBarStyle.MarqueeProgressBar"/>
        ///   or <see cref="Mayfly.TaskDialogs.ProgressBarStyle.None"/>.
        /// </para>
        /// </remarks>
        ...
        public void ReportProgress(int percentProgress, string text, string description, object userState)
        {
            ...
            if( _dialog == null )
                throw new InvalidOperationException(Resources.TaskDialogs.ProgressDialogNotRunningError);
        ...
        private void RunProgressDialog(IntPtr owner, object argument)
        {
            if( _backgroundWorker.IsBusy )
                throw new InvalidOperationException(Resources.TaskDialogs.ProgressDialogRunning);
            ...
            if( Animation != null )
            {
                ...
                {
                    throw new InvalidOperationException(string.Format(System.Globalization.CultureInfo.CurrentCulture, Resources.TaskDialogs.AnimationLoadErrorFormat, ex.Message), ex);
                }
            ...
            Interop.ProgressDialogFlags flags = Mayfly.TaskDialogs.Interop.ProgressDialogFlags.Normal;
            if( owner != IntPtr.Zero )
                flags |= Mayfly.TaskDialogs.Interop.ProgressDialogFlags.Modal;
            switch( ProgressBarStyle )
            {
            case ProgressBarStyle.None:
                flags |= Mayfly.TaskDialogs.Interop.ProgressDialogFlags.NoProgressBar;
                break;
            case ProgressBarStyle.MarqueeProgressBar:
                if( NativeMethods.IsWindowsVistaOrLater )
                    flags |= Mayfly.TaskDialogs.Interop.ProgressDialogFlags.MarqueeProgress;
                else
                    flags |= Mayfly.TaskDialogs.Interop.ProgressDialogFlags.NoProgressBar; // Older than Vista doesn't support marquee.
                break;
            }
            if( ShowTimeRemaining )
                flags |= Mayfly.TaskDialogs.Interop.ProgressDialogFlags.AutoTime;
            if( !ShowCancelButton )
                flags |= Mayfly.TaskDialogs.Interop.ProgressDialogFlags.NoCancel;
            if( !MinimizeBox )
                flags |= Mayfly.TaskDialogs.Interop.ProgressDialogFlags.NoMinimize;
