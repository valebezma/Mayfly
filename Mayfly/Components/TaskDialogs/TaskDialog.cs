...
namespace Mayfly.TaskDialogs
{
    ...
    public partial class TaskDialog : Component, IWin32Window
    {
        ...
        ///   with <see cref="TaskDialog.ButtonStyle"/> set to <see cref="Mayfly.TaskDialogs.TaskDialogButtonStyle.CommandLinks"/>
        ///   or <see cref="Mayfly.TaskDialogs.TaskDialogButtonStyle.CommandLinksNoIcon"/>, you delineate the command from the 
        ...
        /// </summary>
        /// <value>
        /// A <see cref="Mayfly.TaskDialogs.ProgressBarStyle"/> that indicates the type of progress bar shown on the task dialog.
        /// </value>
        /// <remarks>
        /// <para>
        ///   If this property is set to <see cref="Mayfly.TaskDialogs.ProgressBarStyle.MarqueeProgressBar"/>, the marquee will
        ///   scroll as long as the dialog is visible.
        /// </para>
        /// <para>
        ///   If this property is set to <see cref="Mayfly.TaskDialogs.ProgressBarStyle.ProgressBar"/>, the value of the
        ...
        /// <see cref="Mayfly.TaskDialogs.ProgressBarStyle.MarqueeProgressBar"/>.
        /// </remarks>
        ...
        /// <remarks>
        /// This property is only used if the <see cref="ProgressBarStyle"/> property is 
        /// <see cref="Mayfly.TaskDialogs.ProgressBarStyle.ProgressBar"/>.
        /// </remarks>
        ...
        /// </summary>
        /// <value>
        /// A <see cref="Mayfly.TaskDialogs.ProgressBarState"/> indicating the state of the task dialog's progress bar.
        /// The default value is <see cref="Mayfly.TaskDialogs.ProgressBarState.Normal"/>.
        /// </value>
        /// <remarks>
        /// This property is only used if the <see cref="Mayfly.TaskDialogs.ProgressBarStyle"/> property is 
        /// <see cref="Mayfly.TaskDialogs.ProgressBarStyle.ProgressBar"/>.
        /// </remarks>
        ...
        /// <summary>
        /// This method is for internal Mayfly.TaskDialogs use and should not be called from your code.
        /// </summary>
        ...
        {
            if( !IsDialogRunning )
                throw new InvalidOperationException(Resources.TaskDialogs.TaskDialogNotRunningError);
        ...
        private TaskDialogButton ShowDialog(IntPtr owner)
        {
            if( !OSSupportsTaskDialogs )
                throw new NotSupportedException(Resources.TaskDialogs.TaskDialogsNotSupportedError);

            if( IsDialogRunning )
                throw new InvalidOperationException(Resources.TaskDialogs.TaskDialogRunningError);

            if( _buttons.Count == 0 )
                throw new InvalidOperationException(Resources.TaskDialogs.TaskDialogNoButtonsError);
        ...
        private List<NativeMethods.TASKDIALOG_BUTTON> SetupButtons()
        {
            ...
            {
                if( button.Id < 1 )
                    throw new InvalidOperationException(Resources.TaskDialogs.InvalidTaskDialogItemIdError);
                ...
                if( button.ButtonType == ButtonType.Custom )
                {
                    if( string.IsNullOrEmpty(button.Text) )
                        throw new InvalidOperationException(Resources.TaskDialogs.TaskDialogEmptyButtonLabelError);
                    ...
                    if( ButtonStyle == TaskDialogButtonStyle.CommandLinks || ButtonStyle == TaskDialogButtonStyle.CommandLinksNoIcon && !string.IsNullOrEmpty(button.CommandLinkNote) )
                        taskDialogButton.pszButtonText += Constants.Break + button.CommandLinkNote;
        ...
        private void CheckCrossThreadCall()
        {
            ...
            if( handle != IntPtr.Zero )
            {
                ...
                if( windowThreadId != threadId )
                    throw new InvalidOperationException(Resources.TaskDialogs.TaskDialogIllegalCrossThreadCallError);
            }
        }
