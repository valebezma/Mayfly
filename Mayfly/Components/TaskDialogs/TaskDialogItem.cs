...
namespace Mayfly.TaskDialogs
{
    ...
    public abstract partial class TaskDialogItem : Component
    {
        ...
        {
            if( Owner == null )
                throw new InvalidOperationException(Resources.TaskDialogs.NoAssociatedTaskDialogError);
                ...
                if( items != null )
                {
                    foreach( TaskDialogItem item in items )
                    {
                        if( item != this && item != itemToExclude && item.Id == id )
                            throw new InvalidOperationException(Resources.TaskDialogs.DuplicateItemIdError);
                    }
                }
            }
        }
