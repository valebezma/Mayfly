...
{
    public partial class GetIndex : Form
    {
        ...
        public GetIndex(OpenFileDialog openDialog, string defaultName, Uri uri)
        {
            InitializeComponent();
            ...
            Icon extIcon = Service.GetIcon(IO.GetIconSource(Path.GetExtension(DefaultName)), new Size(128, 128));
        ...
        {
            taskDialogMissingReference.CustomMainIcon = Service.GetIcon(IO.GetIconSource(Path.GetExtension(DefaultName)), new Size(32, 32));
        ...
        private void dialogReference_Load(object sender, EventArgs e)
        {
            ...
            {
                FilePath = Path.Combine(OpenDialog.InitialDirectory, DefaultName);
            ...
            else if (tdb == tdbDownload)
            {
                try
                {
                    ...
                    FilePath = Path.Combine(OpenDialog.InitialDirectory, Path.GetFileName(DownloadUri.LocalPath));
