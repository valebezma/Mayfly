...
{
    public partial class WizardExport2 : Form
    {
        ...
        private WizardExport2()
        { 
            InitializeComponent();
        }

        public WizardExport2(Data data)
            : this()
        {
            Data = data.GetStack();
            Allowed = new Data();
        ...
        private void contextGrowthChart_Click(object sender, EventArgs e)
        {
            foreach (DataGridViewRow gridRow in spreadSheetGrowth.SelectedRows)
            {
                ...
                Scatterplot scatter = Allowed.GrowthModels.GetInternalScatterplot(spc).Copy();
