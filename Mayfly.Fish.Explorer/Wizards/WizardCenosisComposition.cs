...
namespace Mayfly.Fish.Explorer.Survey
{
    public partial class WizardCenosisComposition : Form
    {
        ...
        public void Run(WizardGearSet _gearWizard, SpeciesComposition _example)
        {
            ...
            calculatorCenosis.RunWorkerAsync(_example);
        }
        ...
        public void AddSummarySection(Report report)
        {
            ...
            report.AddParagraph(Resources.Reports.Section.Cenosis.Paragraph_1,
                gearWizard.SelectedSamplerType.ToDisplay());

            report.AddEquation("\\text{Abundance} = \\frac{\\overline{CPUE}}{q}", ",");

            report.AddParagraph(
                string.Format(Resources.Reports.Section.Cenosis.Paragraph_2, report.NextTableNumber)
                );

            Report.Table table1 = new Report.Table(Resources.Reports.Section.Cenosis.Table_1);

            table1.StartRow();
            table1.AddHeaderCell(Wild.Resources.Reports.Caption.Species, .25, 2);
            table1.AddHeaderCell(Resources.Reports.Section.Cenosis.ColumnCatchability, 2, CellSpan.Rows);
        ...
        {
            NaturalComposition = (e.Argument == null ? gearWizard.SelectedData.GetCenosisCompositionFrame() : (SpeciesComposition)e.Argument);
        ...
        private void cenosisCalculator_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            for (int i = 0; i < NaturalComposition.Count; i++)
            {
                spreadSheetCenosis[ColumnCommAbundance.Index, i].Value = NaturalComposition[i].Abundance;
                spreadSheetCenosis[ColumnCommAbundanceP.Index, i].Value = NaturalComposition[i].AbundanceFraction;
                spreadSheetCenosis[ColumnCommBiomass.Index, i].Value = NaturalComposition[i].Biomass;
                spreadSheetCenosis[ColumnCommBiomassP.Index, i].Value = NaturalComposition[i].BiomassFraction;
                spreadSheetCenosis[ColumnCommQ.Index, i].Value = 
                    Service.GetCatchability(gearWizard.SelectedSamplerType, NaturalComposition[i].Name);
                spreadSheetCenosis[ColumnCommOccurrence.Index, i].Value = NaturalComposition[i].Occurrence;
                spreadSheetCenosis[ColumnCommDominance.Index, i].Value = NaturalComposition[i].Dominance;
            }
        ...
        private void spreadSheetCenosis_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == ColumnCommQ.Index && spreadSheetCenosis.ContainsFocus)
            {
                if (spreadSheetCenosis[e.ColumnIndex, e.RowIndex].Value == null)
                {
                    spreadSheetCenosis[e.ColumnIndex, e.RowIndex].Value =
                        Service.GetCatchability(gearWizard.SelectedSamplerType, NaturalComposition[e.RowIndex].Name);
                }
                else
                {
                    double value = (double)spreadSheetCenosis[e.ColumnIndex, e.RowIndex].Value;
