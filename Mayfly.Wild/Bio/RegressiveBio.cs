...
{
    public class RegressiveBio : IFormattable, IBio
    {
        public IBioable Parent { get; set; }
        ...
        public string Description
        {
            get
            {
                return string.Format(Resources.Interface.BioFormat, Dates.GetDatesDescription(), Places.Merge());
            }
        }
        ...
        public RegressiveBio(IBioable data, DataRow[] speciesRows,
            DataColumn xColumn, DataColumn yColumn, TrendType type)
        {
        ...
        public PredictionModel GetInternalRegression(string name)
        {
        ...
        {
            PredictionModel regression = GetInternalRegression(name);
            ...
            if (scatter == null || !scatter.IsRegressionOK) // There is no confirmed scatterplot for given species
            {
                ...
                if (VisualConfirmation)
                {
                    ...
                    if (GetConfirmation(confirm) == DialogResult.OK)
                    {
                        ...
                        return this.Inversive ? confirm.Model.Regression.PredictInversed(x) : confirm.Model.Regression.Predict(x);
                    }
                    else
                    {
                        Denied = true;
                        // User closed dialog - do NOT use this Bio!
                        return double.NaN;
                    }
                }
                else
                {
                    if (!scatter.IsRegressionOK) return double.NaN;

                    ReplaceInternalScatterplot(name, scatter);

                    return this.Inversive ? scatter.Regression.PredictInversed(x) : scatter.Regression.Predict(x);
                }
            }
        ...
        public void Involve(RegressiveBio external, bool clear)
        {
            if (clear)
            {
        ...
        public Scatterplot Combine(Scatterplot original, Scatterplot external1)
        {
            ...
            biSample.X.Name = Resources.Common.LengthUnits;
