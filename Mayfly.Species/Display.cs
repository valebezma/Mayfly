...
namespace Mayfly.Species
{
    public partial class Display : Form
    {
        ...
        public SpeciesKey.RankRow SelectedBase
        {
            get
            {
                if (IsBaseNodeSelected)
                {
                    return (SpeciesKey.RankRow)treeViewTaxa.SelectedNode.Tag;
                }

                if (IsTaxaNodeSelected)
                {
                    return (SpeciesKey.RankRow)treeViewTaxa.SelectedNode.Parent.Tag;
                }
                return null;
            }
        }
        ...
        public Display(string filename) : this()
        {
            LoadData(filename);
        }

        public Display(string filename, bool inWork) : this(filename)
        {
            InWork = inWork;
        ...
        public Display(SpeciesKey data)
            : this()
        {
            Text = "Guide";
        ...
        public Display(SpeciesKey data, SpeciesKey.StepRow step)
            : this(data)
        {
        ...
        {
            foreach (SpeciesKey.RankRow baseRow in Data.Rank.Rows)
            {
                TreeNode baseNode = new TreeNode
                {
                    Tag = baseRow,
                    Text = baseRow.RankName
                };
        ...
        private void UpdateOthers()
        {
            ...
            {
                if (!(baseNode.Tag is SpeciesKey.RankRow)) continue;

                TreeNode[] othersNode = baseNode.Nodes.Find("Other_" + baseNode.Name, false);

                int count = ((SpeciesKey.RankRow)baseNode.Tag).Varia.Length;
