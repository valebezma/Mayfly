// WildDataPropertyHandler.cpp : Defines the exported functions for the DLL application.
//
...
#include "PropertyHandlerConsts.h"
...
// Map of property keys to the locations of their value(s) in the Mayfly Card XML schema
struct PROPERTYMAP
{
    const PROPERTYKEY *pkey;    // pointer type to enable static declaration
    PCWSTR pszXPathParent;
    PCWSTR pszValueNodeName;
	VARENUM vePropertyType;
	bool bShitStore;
};

const PROPERTYMAP g_rgPROPERTYMAP[] =
{	
	{&PKEY_Investigator, L"Data/Card",L"Investigator",VT_LPWSTR, false },
	{&PKEY_When, L"Data/Card",L"When",VT_FILETIME, false },
	{&PKEY_Label, L"Data/Card",L"Label",VT_LPWSTR, false },
	{&PKEY_Species, L"Data/Species",L"Species",VT_LPWSTR, true }
};
...
class CWildDataPropertyHandler :
    public IPropertyStore,
    public IPropertyStoreCapabilities,
    public IInitializeWithStream
{
public:
    CWildDataPropertyHandler() : _cRef(1), _pStream(NULL), _grfMode(0), _pDomDoc(NULL), _pCache(NULL)
    {
        DllAddRef();
    }
    ...
    IFACEMETHODIMP QueryInterface(REFIID riid, void **ppv)
    {
        static const QITAB qit[] = {
            QITABENT(CWildDataPropertyHandler, IPropertyStore),
            QITABENT(CWildDataPropertyHandler, IPropertyStoreCapabilities),
            QITABENT(CWildDataPropertyHandler, IInitializeWithStream),
			{ 0 },
        };
...
private:
    ...
    HRESULT _LoadPropertyValues(IXMLDOMNode *pNodeParent, const PROPERTYMAP &map, PROPVARIANT *ppropvar);
    ...
    IXMLDOMDocument*     _pDomDoc; // DOM object representing the Mayfly Card file
...
{
    *ppv = NULL;
    CWildDataPropertyHandler *pNew = new(std::nothrow) CWildDataPropertyHandler;
...
HRESULT CWildDataPropertyHandler::GetCount(DWORD *pcProps)
{
    *pcProps = 0;
    return _pCache ? _pCache->GetCount(pcProps) : E_UNEXPECTED;
}

HRESULT CWildDataPropertyHandler::GetAt(DWORD iProp, PROPERTYKEY *pkey)
{
    *pkey = PKEY_Null;
    return _pCache ? _pCache->GetAt(iProp, pkey) : E_UNEXPECTED;
}

HRESULT CWildDataPropertyHandler::GetValue(REFPROPERTYKEY key, PROPVARIANT *pPropVar)
{
...
HRESULT CWildDataPropertyHandler::SetValue(REFPROPERTYKEY key, REFPROPVARIANT propVar)
{
...
HRESULT CWildDataPropertyHandler::Commit()
{
    HRESULT hr = E_UNEXPECTED;
    if (_pCache)
    {
...
HRESULT CWildDataPropertyHandler::IsPropertyWritable(REFPROPERTYKEY key)
{
    // System.Search.Contents is the only property not supported for writing
    return (key == PKEY_Search_Contents) ? S_FALSE : S_OK;
}

void Debug_Message(char * p_Message)
{
	int a = 0;
	if (a) {
		::MessageBoxA(0, p_Message, "Test", 0);
	}
...
HRESULT CWildDataPropertyHandler::Initialize(IStream *pStream, DWORD grfMode)
{
...
HRESULT CWildDataPropertyHandler::_LoadCacheFromDom()
{
	Debug_Message("_LoadCacheFromDom");
    HRESULT hr = S_OK;
...
HRESULT CWildDataPropertyHandler::_LoadPropertyValues(IXMLDOMNode *pNodeParent,
                                                    const PROPERTYMAP &map,
                                                    PROPVARIANT *ppropvarValues)
{
	Debug_Message("_LoadPropertyValues");
    // intialize the outparam
    PropVariantInit(ppropvarValues);

    // select the value nodes
    IXMLDOMNodeList *pValueList = NULL;
    HRESULT hr = pNodeParent->selectNodes(CastToBSTRForInput(map.pszValueNodeName), &pValueList);
    if (hr == S_OK)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (hr == S_OK)
                {
					/*switch (map.vePropertyType)
					{
					case VT_FILETIME:
						{
							tm v_DateTime;
							char *strptime(const_cast<char*>(pbstrValues[0]), const char *format, struct tm *tm);

							hr = InitPropVariantFromFileTime(const_cast<PCWSTR*>(pbstrValues), cValues, ppropvarValues);
							break;
						}
...
HRESULT CWildDataPropertyHandler::_LoadProperty(const PROPERTYMAP &map)
{
	Debug_Message("_LoadProperty");
	...
	if (!map.bShitStore) 
	{
		...
		if (hr == S_OK)
		{
			...
			hr = _LoadPropertyValues(pNodeParent, map, &propvarValues);
			if (hr == S_OK)
			{
	...
	else
	{
		...
		//Wave1: fixed wrong path
		WCHAR fullPath[MAX_PATH];
		wcscpy_s(fullPath, map.pszXPathParent);
		wcscat_s(fullPath, TEXT("/"));
		wcscat_s(fullPath, map.pszValueNodeName);

		hr = _pDomDoc->selectNodes(CastToBSTRForInput(fullPath), &pValueList);
		if (hr == S_OK)
		{
			long cValues = 0;
			hr = pValueList->get_length(&cValues);
			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					{
						// package the list of values up in a PROPVARIANT
						hr = InitPropVariantFromStringVector(const_cast<PCWSTR*>(pbstrValues), cValues, &propvarValues);
					}
...
HRESULT CWildDataPropertyHandler::_LoadExtendedProperties()
{
	Debug_Message("_LoadExtendedProperties");
    // select the list of extended property nodes
    IXMLDOMNodeList *pList = NULL;
    HRESULT hr = _pDomDoc->selectNodes(CastToBSTRForInput(L"Data/Info/ExtendedProperties/Property"), &pList);
    if (hr == S_OK)
    {
        long cElems = 0;
...
HRESULT CWildDataPropertyHandler::_LoadSearchContent()
{
    ...
    BSTR bstrContentXSLT = SysAllocString(L"<xsl:stylesheet version=\"1.0\" xmlns:xsl=\"http://www.w3.org/1999/XSL/Transform\">\n"
                                          ...
                                          L"  <xsl:template match=\"/\">\n"
											L"    <xsl:apply-templates select=\"Data/Card/Investigator\"/>\n"
											L"    <xsl:apply-templates select=\"Data/Card/When\"/>\n"
											L"    <xsl:apply-templates select=\"Data/Card/Label\"/>\n"
											L"    <xsl:apply-templates select=\"Data/Species/Species\"/>\n"
                                          L"  </xsl:template>\n"
                                          L"  <xsl:template match=\"*\">\n"
...
HRESULT CWildDataPropertyHandler::_SaveCacheToDom()
{
...
HRESULT CWildDataPropertyHandler::_SavePropertyValues(IXMLDOMNode* pNodeParent,
                                                    PCWSTR pszValueNodeName,
...
HRESULT CWildDataPropertyHandler::_SaveProperty(REFPROPVARIANT propvar, const PROPERTYMAP &map)
{
	Debug_Message("_SaveProperty");
...
HRESULT CWildDataPropertyHandler::_SaveExtendedProperty(REFPROPERTYKEY key, REFPROPVARIANT propvar)
{
	Debug_Message("_SaveExtendedProperty");
...
HRESULT CWildDataPropertyHandler::_EnsureChildNodeExists(IXMLDOMNode *pNodeParent, PCWSTR pszName, PCWSTR pszXPath, IXMLDOMNode **ppNodeChild)
{
...
const WCHAR c_szWildDataFileExtension[] = L".bcd";
const WCHAR c_szWildDataProgID[] = L"Mayfly.Benthos";

HRESULT RegisterHandler()
{
    const WCHAR c_szPropertyHandlerDescription[] = L"Mayfly.Benthos";

    // register the property handler COM object with the system
    CRegisterExtension re(__uuidof(CWildDataPropertyHandler), HKEY_LOCAL_MACHINE);
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = re.RegisterPropertyHandler(c_szWildDataFileExtension);
        }
    }

    if (SUCCEEDED(hr))
    {
        hr = re.RegisterProgIDValue(c_szWildDataProgID, L"InfoTip", c_szCardInfoTip);
        if (SUCCEEDED(hr))
        {
            hr = re.RegisterProgIDValue(c_szWildDataProgID, L"FullDetails", c_szCardFullDetails);
            if (SUCCEEDED(hr))
            {
                hr = re.RegisterProgIDValue(c_szWildDataProgID, L"PreviewDetails", c_szCardPreviewDetails);
                if (SUCCEEDED(hr))
                {
                    hr = re.RegisterProgIDValue(c_szWildDataProgID, L"PreviewTitle", c_szCardPreviewTitle);
                }
            }
        }
    }
    ...
    if (SUCCEEDED(hr))
    {
        hr = re.RegisterExtensionWithProgID(c_szWildDataFileExtension, c_szWildDataProgID);
    }
...
HRESULT UnregisterHandler()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = re.UnRegisterPropertyHandler(c_szWildDataFileExtension);
        if (SUCCEEDED(hr))
        {
            ...
            hr = re.UnRegisterProgID(c_szWildDataProgID, c_szWildDataFileExtension);
        }
    }
    return hr;
}